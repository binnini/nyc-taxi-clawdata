...
SLA는 두 가지로 분리하여 측정한다.
- **DuckDB SLA**: 순수 엔진 성능 (인프라 검증)
- **Agent SLA**: LLM 추론 포함 전체 응답 시간 (UX 검증)
  - Agent 응답 = SQL 생성 (LLM) + DuckDB 실행 + 결과 해석 (LLM)

| 쿼리 | 내용 | DuckDB SLA | Agent SLA |
|------|------|-----------|-----------|
| Q1 | vendor_id별 COUNT | 2초 이내 | 15초 이내 |
| Q2 | passenger_count별 AVG(total_amount) | 2초 이내 | 15초 이내 |
| Q3 | passenger_count + 연도별 COUNT | 2초 이내 | 15초 이내 |
| Q4 | passenger_count + 연도 + ROUND(distance) 3중 GROUP BY + ORDER BY | 5초 이내 | 20초 이내 |
| Q5 | pickup_zone JOIN + COUNT (존 분석) | 5초 이내 | 20초 이내 |
...
| 항목 | 측정 방법 |
|------|----------|
| DuckDB 쿼리 실행 시간 | `.timer on` 실측 (`docs/phase1/queries_results.txt`) |
| Agent 전체 응답 시간 | 질문 시각 ~ 응답 완료 시각 수동 측정 |
...
- [x] S3 버킷 생성 및 CloudShell로 parquet 60개 업로드
- [x] EC2 S3 접근 설정 (AWS credentials)
- [x] dbt run 실행 → warehouse.duckdb 적재
- [x] S3 버킷 삭제
- [x] DuckDB 벤치마크 쿼리 Q1~Q5 실행 및 결과 기록
- [ ] ClawData Agent 연동 검증 (Agent SLA Q1~Q5)
