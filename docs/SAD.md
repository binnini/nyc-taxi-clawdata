...
### ADR-03: 인프라 — EC2 t3.medium 선택

**결정**: EC2 t3.medium (2 vCPU, 4GB RAM)

**대안 검토**
| 대안 | 배제 이유 |
|------|----------|
| t3.large (8GB) | 메모리 제약 챌린지 무효화 |
| t3.small (2GB) | Out-of-Core 스필 과다로 SLA 달성 불가 |
| 로컬 머신 | 25GB 데이터 로컬 저장 부담, 재현 환경 불일치 |

**선택 근거**
- 25GB 데이터 / 4GB RAM → Out-of-Core 처리 강제
- DuckDB 스필(Spill) 메커니즘 직접 관찰 가능
- 비용 최소화 (미사용 시 stop으로 과금 중단)

**리스크**
- dbt run 중 OOM 위험 → ADR-05에서 완화
...
**선택 근거**
- t3.medium 네트워크 대역폭(최대 5Gbps)과 S3 API 지연 감안
...
| threads | 2 | t3.medium vCPU 수와 일치, 컨텍스트 스위칭 overhead 방지 |
...
- [ ] t3.medium 네트워크 대역폭으로 dbt run 적재 시간 실측
